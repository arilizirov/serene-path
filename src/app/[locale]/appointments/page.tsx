...
export default async function AppointmentsPage({
  ...
  // Auth gate FIRST — client-only. Replaces the old getCurrentUser/redirect dance;
  // requireRole redirects an unauthenticated or non-client caller to login.
  const { id: userId } = await requireRole("CLIENT", locale);
  const t = await getTranslations("Client");

  const contact = await getUserContact(userId);

  const appointments = (await getMyAppointments(userId)).map((a) => ({
    ...a,
